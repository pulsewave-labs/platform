...
export default function PerformanceClientPage() {
  ...
  if (!data) return (
    <div className="min-h-screen bg-[#08080a] flex items-center justify-center">
      <div className="text-[14px] text-white/50 mono">Failed to load. <button onClick={() => window.location.reload()} className="text-[#00e5a0] underline">Retry</button></div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#08080a] text-white antialiased">
      ...
      <nav className="sticky top-0 z-50 bg-[#08080a]/90 backdrop-blur-xl border-b border-white/[0.04]">
        <div className="max-w-6xl mx-auto px-6 md:px-10 h-14 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[16px] text-white/40 mono tracking-wide">PERFORMANCE</span>
          </div>
          <div className="flex items-center gap-4">
            <Link href="/auth/login" className="text-[14px] text-white/50 hover:text-white/70 transition-colors">Log In</Link>
            <Link href="/auth/signup" className="text-[14px] px-5 py-2 bg-[#00e5a0] text-black rounded-lg font-bold hover:bg-[#00cc8e] transition-colors">Get Signals</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-10 py-12">
        ...
        {/* Hero */}
        <div className="mb-14">
          <p className="text-[14px] text-[#00e5a0]/40 mono tracking-[.15em] mb-3">VERIFIED TRACK RECORD</p>
          ...
          <p className="text-[16px] text-white/50 mb-8">{s.totalTrades} trades · $1,000 fixed risk · 20x leverage · Market Structure</p>
                ...
                <tbody>
                  {data.monthly.map((m: any, i: number) => (
                    <tr key={i} className={'border-t border-white/[0.02] hover:bg-white/[0.01] transition-colors ' + (i % 2 === 0 ? 'bg-[#0a0a0a]' : 'bg-[#0b0b0b]')}>
                      <td className="px-4 py-3 text-[#aaa] font-medium">{m.month}</td>
                      <td className="px-4 py-3 text-[#777]">{m.trades}</td>
                      <td className="px-4 py-3 text-[#00e5a0]/60">{m.wins}</td>
                      <td className="px-4 py-3 text-[#ff4d4d]/60">{m.losses}</td>
                      <td className={'px-4 py-3 text-right font-bold ' + (m.pnl >= 0 ? 'text-[#00e5a0]' : 'text-[#ff4d4d]')}>
                        {m.pnl >= 0 ? '+' : ''}${m.pnl.toLocaleString()}
                      </td>
                      <td className={'px-4 py-3 text-right ' + (m.pnlPct >= 0 ? 'text-[#00e5a0]/50' : 'text-[#ff4d4d]/50')}>
                        {m.pnlPct >= 0 ? '+' : ''}{m.pnlPct.toFixed(1)}%
                      </td>
                      <td className="px-4 py-3 text-right text-[#888]">${m.balance.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="mb-14">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#0c0c0c] border border-white/[0.04] rounded-lg p-6">
              <div className="text-[14px] text-white/70 font-semibold mb-4">Position Sizing Formula</div>
              <div className="bg-white/[0.02] rounded-lg p-4 mono text-[16px] space-y-2 mb-4">
              ...
              <p className="text-[16px] text-[#666] leading-relaxed">Tighter stop = larger position. Wider stop = smaller position. Risk stays constant at $1,000 regardless of setup.</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-14">
          ...
          <div className="flex flex-wrap gap-2 items-center mb-4">
            <select value={filterPair} onChange={e => { setFilterPair(e.target.value); setPage(1) }}
              className="text-[16px] mono bg-[#0c0c0c] border border-white/[0.04] rounded-lg px-3 py-2 text-[#aaa] focus:outline-none focus:border-[#00e5a0]/30">
            ...
            {(filterPair || filterDirection || filterResult) && (
              <button onClick={() => { setFilterPair(''); setFilterDirection(''); setFilterResult(''); setPage(1) }}
                className="text-[14px] mono text-[#555] hover:text-[#888] transition-colors px-2">
                Clear
              </button>
            )}
          </div>
              ...
              <table className="w-full text-[11px] mono">
                ...
                <tbody>
                  {paginated.map((t: Trade, i: number) => (
                    <tr key={i} className={'border-t border-white/[0.02] hover:bg-white/[0.01] transition-colors ' + (i % 2 === 0 ? 'bg-[#0a0a0a]' : 'bg-[#0b0b0b]')}>
                      <td className="px-4 py-3 text-[#777]">{new Date(t.entry_time).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: '2-digit' })}</td>
                      <td className="px-4 py-3 text-[#aaa] font-medium">{t.pair.replace('/USDT', '')}</td>
                      <td className="px-4 py-3">
                        <span className={'px-1.5 py-0.5 rounded text-[14px] font-bold ' + (t.action === 'LONG' ? 'bg-[#00e5a0]/[0.06] text-[#00e5a0]' : 'bg-[#ff4d4d]/[0.06] text-[#ff4d4d]')}>{t.action}</span>
                      </td>
                      <td className="px-4 py-3 text-right text-[#777]">${t.entry_price.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                      <td className="px-4 py-3 text-right text-[#777]">${t.exit_price.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                      <td className="px-4 py-3 text-right text-[#c9a227]/50">${t.risk_amount.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right text-[#555]">${Math.round(t.fees)}</td>
                      <td className={'px-4 py-3 text-right font-bold ' + (t.pnl >= 0 ? 'text-[#00e5a0]' : 'text-[#ff4d4d]')}>
                      ...
                      <td className="px-4 py-3 text-right text-[#888]">${Math.round(t.balance_after).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-4">
              <span className="text-[16px] text-[#555] mono">Page {page} of {totalPages}</span>
              <div className="flex gap-1">
                {[
                  ...
                  <button key={i} onClick={btn.action} disabled={btn.disabled}
                    className="px-3 py-1.5 text-[16px] mono bg-[#0c0c0c] border border-white/[0.04] rounded-lg text-[#777] hover:text-[#aaa] hover:border-white/[0.06] disabled:text-[#333] disabled:hover:border-white/[0.04] transition-colors">
                    {btn.label}
                  </button>
                ))}
              </div>
        ...
        <div className="bg-[#0c0c0c] border border-white/[0.04] rounded-lg p-10 text-center mb-10">
          ...
          <p className="text-[16px] text-white/50 mb-6 max-w-md mx-auto">Public trades are delayed 48 hours. Subscribers get signals the moment they fire — straight to Telegram.</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center max-w-sm mx-auto">
            <Link href="/auth/signup" className="flex-1 py-3 bg-[#00e5a0] text-black text-[14px] font-bold rounded-lg hover:bg-[#00cc8e] transition-colors text-center">
              Get Signals — $149/mo
            </Link>
            <Link href="/" className="flex-1 py-3 border border-white/[0.06] text-white/50 text-[14px] font-semibold rounded-lg hover:border-white/[0.12] hover:text-white/70 transition-all text-center">
              Learn More
            </Link>
          </div>
        </div>
